...
def load_curated_snapshot(csv_path: Path, database_url: str) -> dict[str, int | str]:
...
def _build_upsert_sql(*, include_geom: bool) -> str:
    ...
    """
    updates = [f"{column} = EXCLUDED.{column}" for column in CURATED_COLUMNS if column != "listing_id"]
